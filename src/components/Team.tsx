...
//             <div 
//               key={index}
//               className="bg-[#141414] border border-white/10 rounded-2xl p-6 text-center  animate-fade-in"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
          {teamMembers.map((member, index) => (
            <div 
              key={index}
              className="bg-[#141414] border border-white/10 rounded-2xl p-4 md:p-6 text-center animate-fade-in"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <img 
                src={member.image} 
                alt={member.name}
                className="w-24 h-24 md:w-32 md:h-32 rounded-2xl mx-auto mb-4 object-cover"
              />
              <h3 className="text-lg md:text-xl font-bold text-white mb-2">{member.name}</h3>
              <p className="text-sm md:text-base text-white/70">{member.role}</p>
            </div>
          ))}
        </div>
      </div>
